<template>
  <el-dialog v-model="startFullscreen" :show-close="false" :close-on-click-modal="false"
    :close-on-press-escape="false" title="线上笔试注意事项" width="30%">
    <div class="readMe-container">
      <el-card shadow="never" class="card-inner">
        <span>1.本次考试设有<span style="color: red">视频音频实时监控</span>，请使用带有摄像头、麦克风和扬声器的笔记本或台式电脑（不能使用手机作答）。为确保笔试系统稳定，请使用Google
          Chrome浏览器作答。</span>
        <el-divider />
        <span>2.本次考试使用视频监控，对考生网速要求较高，如果作答过程中，系统提示“当前网络异常无法达到考试要求...”，建议您将及时调整网络。</span>
        <el-divider />
        <span>3.笔试过程中严禁使用计算器，严禁使用草稿纸。严禁抄录、拍摄、录屏、传播试题，严禁查阅资料、书籍、手机等搜索答案，严禁询问他人等所有作弊及协助作弊情形。</span>
        <el-divider />
        ...
        <el-divider />
        <span>8.考生笔试环境应为安静、独立的空间，避免在网吧、咖啡厅、办公室等公共场所进行笔试，避免除考生外其他人员进入监控画面。</span>
        <el-divider />
        <span>9.笔试过程中不允许佩戴耳机，请确保监控画面中头发不要遮挡住耳朵。笔试过程中考生不允许吸烟、吃东西等与笔试无关的行为。</span>
        <el-divider />
    ...
    <template #footer>
      <span class="dialog-footer flex justify-end">
        <el-button style="background-color: red" type="primary" @click="agree" :disabled="isReadFinish">开始考试</el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
const emits = defineEmits(['update:startFullscreen']);
//检测是不是滑到最底下了
