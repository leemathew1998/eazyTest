    ...
    <BasicCardVue>
      ...
      <template #topRight>
        <div class="flex items-center mb-2">
          ...
          <PrintExam v-model:showExamModal="showExamModal" @reLoadData="loadData()">
          </PrintExam>
        </div>
      </template>
      <template #mainContent>
        <div class="h-full -mb-8 flex flex-col justify-between container-examList">
          <el-table :data="tableData.value" stripe style="width: 100%" :max-height="tableHeight"
            ...
            <el-table-column prop="examPaperName" label="试卷名称" min-width="240" />
            ...
            <el-table-column prop="singleTnum" label="单选题" sortable :sortMethod="sortMethod1" min-width="100"
              align="center">
              <template #default="scope">
                {{ `${scope.row.singleTnum}个` }}
              </template>
            </el-table-column>
            <el-table-column prop="moreTnum" label="多选题" sortable :sortMethod="sortMethod2" min-width="100"
              align="center">
              <template #default="scope">
                {{ `${scope.row.moreTnum}个` }}
              </template>
            </el-table-column>
            <el-table-column prop="judgeTnum" label="判断题" sortable :sortMethod="sortMethod3" min-width="100"
              align="center">
              <template #default="scope">
                {{ `${scope.row.judgeTnum}个` }}
              </template>
            </el-table-column>
            <el-table-column prop="ansTnum" label="简答题" sortable :sortMethod="sortMethod4" min-width="100"
              align="center">
              <template #default="scope">
                {{ `${scope.row.ansTnum}个` }}
              </template>
            </el-table-column>
            <el-table-column prop="programTnum" label="编程题" sortable :sortMethod="sortMethod5" min-width="100"
              align="center">
              <template #default="scope">
