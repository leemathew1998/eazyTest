  ...
  <BasicCardVue>
    ...
    <template #topRight>
      <div class="flex items-center mb-2">
        <el-button @click="addUser" v-checkRole="'角色管理-新增'">
          ...
          <el-table-column prop="action" label="操作" fixed="right" min-width="100">
            <template #default="scope">
              <a style="color: #31969a" href="javascript:;" @click="changeInfo(scope.row, false)"
                v-checkRole="'角色管理-修改'">修改</a>
              <el-divider direction="vertical" v-checkRole="'角色管理-删除'" />
              <el-popconfirm title="确定要删除吗？" :teleported="true" @confirm="deleteItem(scope.row)">
                <template #reference>
                  <a style="color: red" href="javascript:;" v-checkRole="'角色管理-删除'">删除</a>
                </template>
              </el-popconfirm>
            </template>
          </el-table-column>
